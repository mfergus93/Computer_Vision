...
# Requirement 1
image_1=cv2.imread(path+'\\images\\animals.png')
image_2=cv2.imread(path+'\\images\\stonehenge.png')
...
cv2.imwrite(path+'\\results\\gray_scale_1.png', gray_scale_1_write)
cv2.imwrite(path+'\\results\\gray_scale_2.png', gray_scale_2_write)

# Requirement 5
gray_scale_mean=(gray_scale_1+gray_scale_2)/2
gray_scale_mean=gray_scale_mean.astype(np.ubyte)
cv2.imwrite(path+'\\results\\gray_scale_mean.png', gray_scale_mean)

# Requirement 6
gray_scale_max=np.maximum(gray_scale_1,gray_scale_2)
gray_scale_max=gray_scale_max.astype(np.ubyte)
cv2.imwrite(path+'\\results\\gray_scale_max.png', gray_scale_max)

# Requirement 7
gray_scale_absdif=np.abs(np.subtract(gray_scale_1,gray_scale_2))
gray_scale_absdif=gray_scale_absdif.astype(np.ubyte)
cv2.imwrite(path+'\\results\\gray_scale_absdif.png', gray_scale_absdif)
